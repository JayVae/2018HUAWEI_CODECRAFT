...
//从大到小排序
            int ii = cpuListLength-1;

            while (!cpuList.isEmpty()){
                Ecs ecsNext = Collections.min(ecsList,new EcsCpuComparator());
                int tmp1 = ecsNext.getCpuNum();
                int tmp2 = ecsNext.getMemNum();
                boolean needNew = true;
//                sort默认从小到大
                Collections.sort(cpuList);
                for (int mm=cpuList.size()-1;mm>=0;mm--){
                    ...
                    if(vmNext<tmp1 && vmCons<tmp2){
                        ecsNext.setCpuNum(tmp1-vmNext);
                        ecsNext.setMemNum(tmp2-vmCons);
                        ecsNext.setNameList(flavorName);
